...
tree_estimator.fit(train_x, train_y)
happiness = pd.DataFrame(tree_estimator.predict(test_abbr), index= test_abbr['id'],columns=["happiness"])
# id = test_abbr['id']
# result = pd.concat([id, happiness['happiness']], axis=1)
print(result)
happiness.to_csv('result.csv')